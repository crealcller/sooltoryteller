...
public class MemberController {
	...
	//kakaoLogin code받기
	/*
	 * @RequestMapping(value = "/Koauth", produces = "application/json", method =
	 ...
	 * System.out.println("kakaologin code:"+code); ModelAndView mav = new
	 * ModelAndView(); JsonNode node =
	 * KakaoLoginController.getKakaoAccessToken(code); JsonNode accessToken =
	 * node.get("access_token"); JsonNode userInfo =
	 * KakaoLoginController.getKakaoUserInfo(accessToken); String kemail = null;
	 * String kname = null; String kimg = null; JsonNode properties =
	 * userInfo.path("propertis"); JsonNode kakao_account =
	 * userInfo.path("kakao_account"); kemail =
	 * kakao_account.path("email").asText(); kname =
	 * properties.path("nickname").asText(); kimg =
	 * properties.path("profile_image").asText();
	 * System.out.println("userInfo:"+userInfo); session.setAttribute("email",
	 * kemail); session.setAttribute("name", kname); session.setAttribute("img",
	 * kimg); mav.setViewName("home"); return mav;
	 * 
	 * }
	 */
